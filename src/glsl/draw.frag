...
void main(){
    float color = texture(state, gl_FragCoord.xy / screenSize).r;
    fragColor=vec4(color,color,color,1.0);
    float radius = u_rad -0.45;

    if(line_dist(u_org, u_end, gl_FragCoord.xy) <= radius)
    {
        fragColor=vec4(u_val,u_val,u_val,1.0);
        return;
    }
    ...
    if(gl_FragCoord.x <= radius)
        dx = screenSize.x;
    else if(gl_FragCoord.x >= screenSize.x - radius)
        dx = -screenSize.x;

    if(dx!=0.0 && line_dist(u_org, u_end, gl_FragCoord.xy + vec2(dx,0)) <= radius)
    {
        fragColor=vec4(u_val,u_val,u_val,1.0);
        return;
    }

    if(gl_FragCoord.y <= radius)
        dy = screenSize.y;
    else if(gl_FragCoord.y >= screenSize.y - radius)
        dy = -screenSize.y;

    if(dy!=0.0 && line_dist(u_org, u_end, gl_FragCoord.xy + vec2(0,dy)) <= radius)
    {
        fragColor=vec4(u_val,u_val,u_val,1.0);
        return;
    }

    if(dy!=0.0 && dx!=0.0 && line_dist(u_org, u_end, gl_FragCoord.xy + vec2(dx,dy)) <= radius)
    {
        fragColor=vec4(u_val,u_val,u_val,1.0);
        return;
    }
